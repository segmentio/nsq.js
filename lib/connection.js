...
Connection.prototype.connect = function(){
  ...
  debug('%s - connect V%s', addr, version);
  ...
  this.sock.on('error', function(err){
    ...
    err.address = addr;
    err.reconnecting = !!self.reconnecting;
    error(err, self.callbacks);
    self.emit('error', err);
  });

  this.sock.on('connect', function(){
    debug('%s - socket connected', addr);
    self.sock.write('  V' + version);
    self.identify(function(err, features){
...
Connection.prototype.identify = function(fn){
  var features = JSON.stringify({
    feature_negotiation: true,
    short_id: hostname.split('.')[0],
    long_id: hostname,
    user_agent: this.ua,
    msg_timeout: this.opts.msgTimeout
  });
  debug('%s - identify %s', this.addr, features);
  this.command('IDENTIFY', null, features, fn);
};
