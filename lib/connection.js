...
Connection.prototype.broadcastError = function(err){
  debug('socket error: %s', err.message);
  this.callbacks.forEach(function(fn){ fn(err) });
};
...
Connection.prototype.connect = function(fn){
  ...
  this.sock.on('error', this.broadcastError.bind(this));
...
Connection.prototype.onframe = function(frame){
  switch (frame.type) {
    ...
    case 1:
      ...
      debug('error %s', res);
      var fn = this.callbacks.shift();
      if (fn) fn(new Error(res))
      else this.emit('error', new Error(res));
      break;
